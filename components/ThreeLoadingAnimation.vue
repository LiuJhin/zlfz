  ...
  <div class="loading-overlay" v-if="isLoading">
    ...
    <div class="loading-text">
      <div class="loading-title">
        ...
        <span class="letter" style="--delay: 1.0s;">K</span>
      </div>
...
const createHexagonLogo = () => {
  ...
  fontLoader.load('/node_modules/three/examples/fonts/helvetiker_bold.typeface.json', (font) => {
    ...
    const textWidthZ = textGeometryZ.boundingBox.max.x - textGeometryZ.boundingBox.min.x;
    ...
    const textWidthA = textGeometryA.boundingBox.max.x - textGeometryA.boundingBox.min.x;
    ...
    const letterConnector = new THREE.Points(particlesGeometry, particlesMaterial);
...
const animate = () => {
  ...
  if (hexagonGroup) {
    ...
    hexagonGroup.children.forEach((child, index) => {
      if (child.isMesh) {
        // 字母上下浮动 - 增强效果
        child.position.y = -1.5 + Math.sin(elapsedTime * 0.7 + index * 0.5) * 0.2;
        ...
        // 添加发光强度变化
        if (child.material) {
          child.material.emissiveIntensity = 0.6 + Math.sin(elapsedTime * 0.8 + index) * 0.3;
        }
      }
      ...
      if (child.isPoints) {
        ...
        for (let i = 0; i < positions.length / 3; i++) {
          ...
          positions[i3 + 1] = -1.5 + Math.sin(t * Math.PI + elapsedTime * 1.5) * 1.2;
  ...
  if (particleSystem) {
    ...
    for (let i = 0; i < positions.length / 30; i++) { // 只处理部分粒子以提高性能
      const i3 = i * 3;
...
.loading-title .letter {
  ...
  position: relative;
  animation: 
    letter-appear 0.8s forwards cubic-bezier(0.2, 0.8, 0.2, 1),
    letter-float 3s infinite ease-in-out,
    letter-pulse 2s infinite ease-in-out,
    letter-glow 4s infinite ease-in-out;
  animation-delay: var(--delay), calc(var(--delay) + 0.8s), calc(var(--delay) + 0.8s), calc(var(--delay) + 0.8s);
  opacity: 0;
...
.loading-title .letter::before {
  ...
  width: 2px;
  height: 10px;
  background: linear-gradient(to bottom, rgba(130, 71, 229, 0), rgba(130, 71, 229, 0.8));
...
.loading-title .letter::after {
  ...
  width: 100%;
  height: 2px;
  background: linear-gradient(to right, rgba(130, 71, 229, 0), rgba(130, 71, 229, 0.8), rgba(130, 71, 229, 0));
...
.loading-subtitle {
  ...
  animation: fade-pulse 3s infinite ease-in-out, float 6s infinite ease-in-out reverse;
  position: relative;
  padding: 10px 20px;
  margin-top: 10px;
  overflow: hidden;
...
.loading-subtitle::before {
  ...
  width: 100%;
  height: 100%;
  background: linear-gradient(90deg, transparent, rgba(130, 71, 229, 0.2), transparent);
...
@keyframes letter-glow {
  0% { text-shadow: 0 0 30px rgba(130, 71, 229, 0.6); }
  25% { text-shadow: 0 0 35px rgba(130, 71, 229, 0.8), 0 0 10px rgba(255, 255, 255, 0.3); }
  50% { text-shadow: 0 0 40px rgba(130, 71, 229, 1), 0 0 20px rgba(255, 255, 255, 0.5); }
  75% { text-shadow: 0 0 35px rgba(130, 71, 229, 0.8), 0 0 10px rgba(255, 255, 255, 0.3); }
  100% { text-shadow: 0 0 30px rgba(130, 71, 229, 0.6); }
}

@keyframes letter-spark {
  0% { opacity: 0; height: 0; }
  50% { opacity: 1; height: 15px; }
  100% { opacity: 0; height: 0; top: -20px; }
}
